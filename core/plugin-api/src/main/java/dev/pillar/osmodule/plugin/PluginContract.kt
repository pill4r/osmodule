...
object PluginContract {
    ...
    const val HOST_PACKAGE = "dev.pillar.osmodule"
    const val BIND_ACTION = "dev.pillar.osmodule.plugin.BIND"
    const val BIND_PERMISSION = "dev.pillar.osmodule.permission.BIND_PLUGIN"
    const val PERMISSION_CENTER_ACTION = "dev.pillar.osmodule.plugin.MANAGE_PERMISSIONS"
    ...
    const val BOOTSTRAP_PROVIDER_CLASS =
        "dev.pillar.osmodule.plugin.PluginBootstrapProvider"
    ...
    const val METADATA_ID = "dev.pillar.osmodule.plugin.ID"
    const val METADATA_NAME = "dev.pillar.osmodule.plugin.NAME"
    const val METADATA_VERSION = "dev.pillar.osmodule.plugin.VERSION"
    const val METADATA_PROTOCOL_MIN = "dev.pillar.osmodule.plugin.PROTOCOL_MIN"
    const val METADATA_PROTOCOL_MAX = "dev.pillar.osmodule.plugin.PROTOCOL_MAX"
    const val METADATA_CAPABILITIES = "dev.pillar.osmodule.plugin.CAPABILITIES"
    ...
    /** Optional already-connected camera AP; receivers must fall back to their own join if absent. */
    const val KEY_CAMERA_NETWORK = "camera_network"
    ...
    /** Optional; absent means the legacy dual-fisheye video source. */
    const val KEY_MEDIA_SOURCE_KIND = "media_source_kind"
    const val MEDIA_SOURCE_DUAL_FISHEYE_VIDEO = "dual-fisheye-video"
    const val MEDIA_SOURCE_EQUIRECTANGULAR_IMAGE = "equirectangular-image"

    const val RSDK_PACKAGE = "dev.pillar.osmodule.plugin.rsdk"
    const val RSDK_PLUGIN_ID = "rsdk-control"
    const val POCKET4P_PACKAGE = "dev.pillar.osmodule.plugin.pocket4p"
    const val POCKET4P_PLUGIN_ID = "pocket4p-control"
    const val PANORAMA_PACKAGE = "dev.pillar.osmodule.plugin.panorama360"
