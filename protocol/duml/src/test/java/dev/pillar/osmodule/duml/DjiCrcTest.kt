...
class DjiCrcTest {
    ...
    private val crc16Table = intArrayOf(
    ...
    private fun tableCrc8(data: ByteArray): Int {
        var crc = 0x77
        for (b in data) crc = crc8Table[(crc xor (b.toInt() and 0xFF)) and 0xFF]
        return crc
    }

    private fun tableCrc16(data: ByteArray): Int {
        var crc = 0x3692
        for (b in data) crc = (crc ushr 8) xor crc16Table[(crc xor (b.toInt() and 0xFF)) and 0xFF]
    ...
    fun crc8CrossVerify() {
        val vectors = arrayOf(
            byteArrayOf(0x55, 0x33, 0x04),
            byteArrayOf(0x55, 0x0D, 0x04),
            byteArrayOf(0x00),
            byteArrayOf(0xFF.toByte()),
            byteArrayOf(0x01, 0x02, 0x03, 0x04, 0x05),
            ByteArray(256) { it.toByte() },
        )
        for (data in vectors) assertEquals(tableCrc8(data), DjiCrc.computeCrc8(data))
    }

    @Test
    fun crc16CrossVerify() {
        val vectors = arrayOf(
            byteArrayOf(0x55, 0x0D, 0x04, 0x69, 0x02, 0x08, 0x2C, 0x8C.toByte(), 0x40, 0x08, 0x78),
            byteArrayOf(0x00),
            byteArrayOf(0xFF.toByte()),
            ByteArray(256) { it.toByte() },
        )
        for (data in vectors) assertEquals(tableCrc16(data), DjiCrc.computeCrc16(data))
    }

    @Test
    fun crc8KnownValue() {
        assertEquals(0xC2, DjiCrc.computeCrc8(byteArrayOf(0x55.toByte(), 0x33, 0x04)))
    }
