...
class PocketZoomVelocityTest {
    ...
    @Test
    fun `held rocker keeps sending after its local target reaches the boundary`() {
        assertEquals(
            true,
            PocketZoomVelocity.shouldEmit(12.0, 12.0, 3.0, 50L),
        )
        assertEquals(
            false,
            PocketZoomVelocity.shouldEmit(12.0, 12.0, 0.0, 50L),
        )
    }
