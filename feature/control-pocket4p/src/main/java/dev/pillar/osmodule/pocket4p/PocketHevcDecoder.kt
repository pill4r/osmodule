...
internal class PocketHevcDecoder(
    ...
    interface Listener {
        ...
        /** Decoder lost a reference picture and needs the camera to cut a fresh GOP. */
    ...
    fun decode(accessUnit: ByteArray): Boolean = synchronized(lock) {
        ...
        val wasAwaitingIrap = awaitingIrap
        ...
        if (!queued) {
            ...
            if (shouldRequestFreshIrap(wasAwaitingIrap, isIrap, queued)) {
                Log.w(TAG, "HEVC inter-frame was not queued; requesting a fresh camera GOP")
                listener.onKeyFrameRequired()
            }
        }
        queued
    }
    ...
    private fun queueLocked(
        ...
        return try {
            ...
            if (index < 0) {
                Log.w(TAG, "HEVC decoder input buffer timed out (keyframe=$keyframe)")
                return false
            }
    ...
    companion object {
        ...
        // A mode change delivers the new SPS/IRAP and its first dependent frames as a short burst.
        // Four milliseconds exhausted Qualcomm's four input buffers before its output thread could
        // return one, dropping a reference P-frame and leaving the decoder waiting forever. One
        // frame interval keeps that burst lossless without adding steady-state pacing.
        private const val INPUT_WAIT_US = 40_000L
        ...
        /** Request one camera GOP exactly when a running decoder first loses a dependent frame. */
        internal fun shouldRequestFreshIrap(
            wasAwaitingIrap: Boolean,
            isIrap: Boolean,
            queued: Boolean,
        ): Boolean = !queued && !isIrap && !wasAwaitingIrap
    }
}
