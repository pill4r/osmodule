...
import dev.pillar.osmodule.duml.PocketGimbalTelemetry
...
/** Borrows or joins camera Wi-Fi, then owns the local lease and one [Pocket4pSession]. */
internal class Pocket4pRemoteController(
    ...
    private val reuseBoundNetwork: Boolean,
    ...
    fun connect(): Boolean {
        if (ssid.isBlank() && !reuseBoundNetwork) {
        ...
        requestOwner(requestGeneration, retry = 0)
        return true
    }

    private fun requestOwner(requestGeneration: Int, retry: Int) {
        if (synchronized(stateLock) { pendingOwnerGeneration != requestGeneration }) return
        val request = CameraSessionOwnerClient.acquireAsync(
            ...
            onOwnerAcquired(requestGeneration, retry, result)
        }
    ...
    private fun onOwnerAcquired(
        ...
        when (result) {
            ...
            is CameraSessionOwnerResult.Busy -> {
                val shouldRetry = reuseBoundNetwork && retry < OWNER_HANDOFF_RETRIES &&
                    synchronized(stateLock) {
                        if (pendingOwnerGeneration != requestGeneration) false
                        else {
                            ownerAcquire = null
                            true
                        }
                ...
                if (shouldRetry) {
                    main.postDelayed(
                        { requestOwner(requestGeneration, retry + 1) },
                        OWNER_HANDOFF_RETRY_MS,
                    )
                } else {
                    failPendingOwner(
                        requestGeneration,
    ...
    private fun startAfterOwnerAcquired(
        ...
        val joiner = ApJoiner(appContext, object : ApJoiner.Listener {
        ...
        if (!reuseBoundNetwork) next.joiner = joiner
        ...
        if (reuseBoundNetwork) {
            next.networkStarted.set(true)
            deliverLog("Pocket 4P: reusing Base camera Wi-Fi connection")
            if (updateAttemptState(next) {
                    it.copy(phase = Pocket4pConnectionPhase.OPENING_DATALINK, error = null)
                }
            ) {
            ...
            runCatching {
                joiner.join(ssid, passphrase, wpa3)
            }.onFailure { error ->
    ...
    fun requestFreshLiveView(): Boolean = activeSession()?.requestFreshLiveView() ?: false
    ...
    private companion object {
        ...
        const val OWNER_HANDOFF_RETRIES = 20
        const val OWNER_HANDOFF_RETRY_MS = 75L
    }
}
