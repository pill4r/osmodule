    ...
    private fun recoverLiveViewIfNeeded(nowMs: Long) {
        when (liveViewRecovery.tick(nowMs)) {
            ...
            PocketLiveViewRecoveryGate.Action.RESEND_ENABLE -> {
                // Keep the last presented image. Only discard the unfinished UDP AU and make the
                // decoder wait for the clean IRAP requested by 09/A8.
                depacketizer.reset()
                listener.onLiveViewRestartRequested()
                send(PocketRemoteCommands.prepareLiveView())
                send(PocketRemoteCommands.enableLiveView())
                Log.i(TAG, "HEVC stalled; requested a fresh live-view GOP")
                listener.onLog("Pocket 4P: HEVC stalled; requested a fresh live-view GOP")
            }
    ...
    private fun processIncoming(datagrams: List<ByteArray>) {
        datagrams.forEach { datagram ->
            if (datagram.size > 20 && (datagram[6].toInt() and 0xFF) == 0x02) {
                liveViewRecovery.onVideo(monotonicMs())
                depacketizer.feed(datagram)?.let(listener::onAccessUnit)
                return@forEach
            }
