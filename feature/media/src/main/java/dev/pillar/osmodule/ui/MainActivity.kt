...
import dev.pillar.osmodule.ble.Brand
import dev.pillar.osmodule.ble.CameraModel
import dev.pillar.osmodule.ble.GattClient
import dev.pillar.osmodule.ble.OsmoScanner
import dev.pillar.osmodule.camera.PathAddressing
import dev.pillar.osmodule.core.CameraFile
import dev.pillar.osmodule.core.CameraStatus
import dev.pillar.osmodule.core.SavedCameras
...
import dev.pillar.osmodule.net.HttpClient
import dev.pillar.osmodule.camera.CameraSession
import dev.pillar.osmodule.core.MediaSession
import dev.pillar.osmodule.core.previewCandidates
import dev.pillar.osmodule.drone.DronePairing
import dev.pillar.osmodule.drone.DroneSession
...
import dev.pillar.osmodule.net.VideoSaveDirectory
import com.google.android.material.button.MaterialButton
import dev.pillar.osmodule.modules.CameraExclusiveController
import dev.pillar.osmodule.modules.CameraExclusiveState
...
import dev.pillar.osmodule.panorama.render.PanoramaCalibrationCodec
...
class MainActivity : AppCompatActivity(), OsmoScanner.Listener, GattClient.Listener {
    ...
    private var restoreGalleryAfterRemote = false
    private var gallerySessionRestoring = false
    private var galleryAdapterAddress: String? = null
    ...
    private var activeCameraNetwork: Network? = null
    ...
    private val keepalive = object : Runnable {
        override fun run() {
            ...
            gattClient?.writeCommand(
                dev.pillar.osmodule.duml.OsmoCommands.sessionPing(
                    dev.pillar.osmodule.duml.OsmoCommands.SESSION_KEEPALIVE
                )
            )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Legacy in-process exclusive mode hook. Cross-process transports acquire the single
        // Base-owned CameraSessionOwnerProvider lease immediately before connecting.
    ...
    override fun onResume() {
        ...
        if (restoreGalleryAfterRemote && ::selectorGroup.isInitialized) {
            resumeGalleryWithSharedWifi()
        } else if (returnToSelectorAfterRemote && ::selectorGroup.isInitialized) {
    ...
    override fun onStop() {
        ...
        invalidatePendingCameraActions(preserveWifiFlow = restoreGalleryAfterRemote)
        super.onStop()
    ...
    private fun shouldPollCameras(): Boolean =
        cameraPolling && activityStarted && ::selectorGroup.isInitialized &&
            selectorGroup.visibility == View.VISIBLE && !connecting &&
    ...
    /**
     * Remote control is an action on the connected camera, not a selector mode. The Pocket plugin
     * borrows Base's already-connected Wi-Fi while media and remote DUML sessions remain exclusive.
     * Other models retain the full disconnect/reconnect fallback.
     */
    private fun openRemoteForConnectedCamera() {
        ...
        if (downloadRunning) {
            toast(getString(R.string.download_running))
            return
        }
        ...
        returnToSelectorAfterRemote = false
        restoreGalleryAfterRemote = false
    ...
    private fun launchRemoteForConnectedCamera(
        ...
        val target = CameraRemoteTarget(
            ...
            network = activeCameraNetwork.takeIf {
                deviceModel == DeviceModels.OSMO_POCKET_4_PRO && wifiUp && apJoiner != null
            },
        ...
        val sharesWifi = target.network != null
        logLine(
            "Remote control: releasing media protocol before plugin handoff " +
                "($handoffDetail, Wi-Fi=${if (sharesWifi) "shared" else "reconnect"}).",
        )
        teardownOffload(preserveWifi = sharesWifi)
        val panelGeneration = ++remoteLaunchGeneration
        remoteLaunchPending = true
        restoreGalleryAfterRemote = sharesWifi
        returnToSelectorAfterRemote = !sharesWifi
            ...
            if (completionDelivered.compareAndSet(false, true)) {
                main.post {
                    ...
                    if (opened) {
                        ...
                        returnToSelectorAfterRemote = false
                        if (sharesWifi) {
                            restoreGalleryAfterRemote = true
                            resumeGalleryWithSharedWifi()
                        } else {
                            switchToSelector()
                        }
    ...
    /** Re-open only the media protocol after Pocket remote returns; the AP request stays alive. */
    private fun resumeGalleryWithSharedWifi() {
        if (cameraSessionOwnerAcquire != null || cameraSessionLease != null) return
        returnToSelectorAfterRemote = false
        val address = currentAddress
        val network = activeCameraNetwork
        val connectivity = getSystemService(ConnectivityManager::class.java)
        val networkReady = network != null && apJoiner != null &&
            connectivity.getNetworkCapabilities(network)
                ?.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) == true
        if (address == null || !networkReady) {
            logLine("Remote control: shared Wi-Fi is no longer available; returning to camera list.")
            switchToSelector()
            return
        }

        showGalleryDuringRemoteReturn(address)
        offloadMode = true
        offloadTriggered = true
        // This is a protocol-owner handoff on an already retained Wi-Fi network, not a new camera
        // connection. Keep the gallery interactive while the media session refreshes, and allow a
        // Remote tap to supersede/cancel that refresh through teardownOffload().
        connecting = false
        wifiUp = true
        datalinkStarted = true
        requestGalleryOwnershipAfterRemote(address, retry = 0)
    }

    /** Reveal the album immediately; only its protocol session refreshes in the background. */
    private fun showGalleryDuringRemoteReturn(address: String) {
        if (!address.equals(galleryAdapterAddress, ignoreCase = true)) {
            adapter = null
            grid.adapter = null
            galleryAdapterAddress = null
        }
        if (adapter == null) {
            loadGallerySnapshot(address)?.let { cached ->
                showGrid(cached, preserveFilters = true)
                logLine("Remote control: restored ${cached.size} cached media items immediately.")
            }
        }
        switchToGrid()
        statusPill.render(
            pillName(),
            getString(R.string.connected_wifi),
            currentStatus,
            showPower = isNano(),
        )
        gallerySessionRestoring = true
        btnRemote.isEnabled = cameraModeController?.state?.locked != true
        btnRemote.alpha = if (btnRemote.isEnabled) 1f else 0.4f
        if (adapter == null) showGallerySyncingState()
        else findViewById<View>(R.id.emptyGallery).visibility = View.GONE
    }

    private fun requestGalleryOwnershipAfterRemote(address: String, retry: Int) {
        ...
        val request = CameraSessionOwnerClient.acquireAsync(
            ...
            finishGalleryOwnershipAfterRemote(requestGeneration, address, retry, acquired)
        }
        if (requestGeneration == cameraSessionOwnerGeneration && offloadMode &&
    ...
    private fun finishGalleryOwnershipAfterRemote(
        requestGeneration: Long,
        address: String,
        retry: Int,
        ...
        if (acquired is CameraSessionOwnerResult.Busy && retry < REMOTE_OWNER_HANDOFF_RETRIES) {
            if (retry == 0) {
                logLine("Remote control: waiting for the plugin to release its protocol session.")
            }
            main.postDelayed({
                if (requestGeneration == cameraSessionOwnerGeneration && offloadMode &&
                    activityStarted && currentAddress == address
                ) {
                    requestGalleryOwnershipAfterRemote(address, retry + 1)
                }
            }, REMOTE_OWNER_HANDOFF_RETRY_MS)
            return
        }
        ...
        val processLease = when (acquired) {
            ...
            is CameraSessionOwnerResult.Busy -> {
                logLine("Remote control: plugin did not release the camera session in time.")
                toast(getString(R.string.camera_session_busy))
                switchToSelector()
                return
            }
            is CameraSessionOwnerResult.Unavailable -> {
                logLine("Remote control: camera-session arbiter unavailable (${acquired.reason}).")
                toast(getString(R.string.external_module_check_failed, acquired.reason))
                switchToSelector()
                return
            }
        }
        when (val local = CameraSessionCoordinator.acquire(
            ownerId = MEDIA_SESSION_OWNER,
            cameraAddress = address,
            ...
            is CameraLeaseResult.Granted -> {
                ...
                cameraSessionLease = local.lease
            }
            is CameraLeaseResult.Busy -> {
                processLease.close()
                logLine("Remote control: local media session is still busy (${local.active.ownerId}).")
                toast(getString(R.string.camera_session_busy))
                switchToSelector()
                return
            }
        ...
        logLine("Remote control: camera Wi-Fi retained; refreshing the media protocol in background.")
        startDatalink(preserveFilters = true)
    }
    ...
    /** Drop the live media protocol. A Pocket remote handoff may retain only the Wi-Fi request. */
    private fun teardownOffload(
        clearCardAction: Boolean = true,
        preserveWifi: Boolean = false,
    ) {
        if (clearCardAction) pendingCardRemoteAddress = null
        invalidatePendingCameraActions(preserveWifiFlow = preserveWifi)
        stopKeepalive()
        dev.pillar.osmodule.net.PreviewNav.clear()
        ...
        dev.pillar.osmodule.net.Highlights.provider = null
        if (!preserveWifi) {
            apJoiner?.release()
            apJoiner = null
            activeCameraNetwork = null
        }
    ...
    private fun invalidatePendingCameraActions(preserveWifiFlow: Boolean = false) {
        ...
        if (!preserveWifiFlow) wifiFlowGeneration++
    ...
    private fun pairIdentity(): String =
        ...
        else dev.pillar.osmodule.duml.DjiPairMessagePayload.DEFAULT_IDENTIFIER
    ...
    private fun onPaired() {
        ...
        val c = dev.pillar.osmodule.duml.OsmoCommands
    ...
    private fun maybeStartOffload() {
        ...
        if (offloadPass.isEmpty()) {
            ...
            gattClient?.writeCommand(
                dev.pillar.osmodule.duml.OsmoCommands.connectWifi(offloadSsid, offloadPass)
            )
        } else {
    ...
    private fun startWifiFlow(ssid: String, pass: String) {
        ...
        val joiner = ApJoiner(this, object : ApJoiner.Listener {
            ...
            override fun onFailed(reason: String) {
                logLine(reason)
                main.post {
                    if (flowGeneration == wifiFlowGeneration) activeCameraNetwork = null
            ...
            override fun onNetwork(network: Network, link: LinkProperties?) {
                ...
                main.post {
                    ...
                    if (shouldLaunchRemoteBeforeMediaListing(
                            requestedAddress = pendingCardRemoteAddress,
                            currentAddress = currentAddress,
                            deviceModel = currentModel.moduleKey,
                        )
                    ) {
                        // A Pocket card's Remote action needs the AP credentials and Network, but no
                        // media manifest. Hand off as soon as Wi-Fi is ready so the gallery is never
                        // constructed or displayed on the way to the remote panel.
                        rememberCurrentCamera()
                        logLine("Remote control: camera Wi-Fi ready; skipping media list and opening Pocket remote directly.")
                        openRemoteForConnectedCamera()
                        return@post
                    }
                    datalinkStarted = true
                    startDatalink()
    ...
    private fun startDatalink(preserveFilters: Boolean = false) {
                if (!offloadMode || isFinishing || isDestroyed) return
                ...
                Thread {
                    ...
                    fun open(m: CameraModel): Pair<MediaSession, List<CameraFile>>? {
                        ...
                        val c: MediaSession =
                            if (m.isDrone) DroneSession(::logLine, m.datalinkPort, bleDroneSerial)
                                ...
                                port = m.datalinkPort,
                                tcpPoke = m.tcpPoke,
                                preferControlPlayback =
                                    m.moduleKey == DeviceModels.OSMO_POCKET_4_PRO,
                            )
                        c.onStatus = { status ->
                        ...
                        c.onFetchProgress = { progress ->
                            if (datalinkSessions.isCurrent(gen)) main.post {
                                if (datalinkSessions.isCurrent(gen) && !preserveFilters) {
                    ...
                    fun superseded(dl: dev.pillar.osmodule.core.MediaSession): Boolean {
                        ...
                        main.post {
                            if (datalinkSessions.isCurrent(gen)) {
                                onPlaybackUnavailable(
                                    keepVisible = preserveFilters && adapter != null,
                                )
                            }
                        }
                        return@Thread
                    ...
                    if (!datalinkSessions.promote(gen, dl) { promoted ->
                            ...
                            dev.pillar.osmodule.net.Highlights.provider = { h ->
                    ...
                    main.post {
                        if (datalinkSessions.isCurrent(gen) && !isFinishing && !isDestroyed) {
                            showGrid(fixed, preserveFilters)
                        }
                    }
                }.start()
    }

    private fun onPlaybackUnavailable(keepVisible: Boolean = false) {
        if (isFinishing || isDestroyed) return
        setConnectProgress(0)
        if (keepVisible) {
            gallerySessionRestoring = false
            btnRemote.isEnabled = cameraModeController?.state?.locked != true
            restoreEmptyGalleryState()
            logLine("Remote control: background media refresh failed; keeping cached gallery visible.")
            toast(getString(R.string.gallery_refresh_failed))
            return
        }
        AlertDialog.Builder(this)
    ...
    private fun showGrid(files: List<CameraFile>, preserveFilters: Boolean = false) {
        ...
        currentAddress?.let { address ->
            galleryAdapterAddress = address.uppercase()
            saveGallerySnapshot(address, files)
        }
        ...
        dev.pillar.osmodule.net.PreviewNav.isQueued = { p -> ad.isQueuedPath(p) }
        dev.pillar.osmodule.net.PreviewNav.trimFor = { p -> ad.trimForPath(p) }
        dev.pillar.osmodule.net.PreviewNav.setQueued = { p, q, t, m -> ad.setQueuedByPath(p, q, t, m) }
    ...
    private fun rememberCurrentCamera() {
        currentAddress?.let {
    ...
    private fun showGallerySyncingState() {
        findViewById<View>(R.id.emptyGalleryIcon).visibility = View.GONE
        findViewById<View>(R.id.emptyGalleryProgress).visibility = View.VISIBLE
        findViewById<TextView>(R.id.emptyGalleryTitle).setText(R.string.gallery_syncing_title)
        findViewById<TextView>(R.id.emptyGalleryMessage).setText(R.string.gallery_syncing_message)
        findViewById<View>(R.id.emptyGallery).visibility = View.VISIBLE
    }

    private fun restoreEmptyGalleryState() {
        findViewById<View>(R.id.emptyGalleryIcon).visibility = View.VISIBLE
        findViewById<View>(R.id.emptyGalleryProgress).visibility = View.GONE
        findViewById<TextView>(R.id.emptyGalleryTitle).setText(R.string.empty_gallery_title)
        findViewById<TextView>(R.id.emptyGalleryMessage).setText(R.string.empty_gallery_message)
    }

    private fun saveGallerySnapshot(address: String, files: List<CameraFile>) {
        runCatching {
            getSharedPreferences(GALLERY_CACHE_PREFS, MODE_PRIVATE).edit()
                .putString(galleryCacheKey(address), GallerySnapshotCache.encode(files))
                .apply()
        }.onFailure { logLine("Gallery cache write failed (${it.javaClass.simpleName}).") }
    }

    private fun loadGallerySnapshot(address: String): List<CameraFile>? = runCatching {
        val encoded = getSharedPreferences(GALLERY_CACHE_PREFS, MODE_PRIVATE)
            .getString(galleryCacheKey(address), null)
        GallerySnapshotCache.decode(encoded)
    }.getOrNull()

    private fun galleryCacheKey(address: String): String =
        "camera_" + address.uppercase().replace(':', '_')
    ...
    private fun resolveStorage(f: CameraFile): Int {
        ...
        val bit = dev.pillar.osmodule.core.StorageRules.mountGuess(false, f.handle, f.cmdHandle)
    ...
    private fun launchPreview(f: CameraFile, group: List<CameraFile>) {
        ...
        dev.pillar.osmodule.net.PreviewNav.items = ad.visibleFiles()
    ...
    override fun onReady(gatt: GattClient) {
        ...
        val woke = gatt.writeCommand(
            dev.pillar.osmodule.duml.OsmoCommands.sessionPing(
                dev.pillar.osmodule.duml.OsmoCommands.SESSION_WAKE
            )
        )
        logLine("READY — sent session wake 0x00/0x2b[04 00] ok=$woke")
        main.postDelayed({
            val frame = dev.pillar.osmodule.duml.OsmoCommands.setPairingPin(pairPin, identifier = pairIdentity())
        ...
        for (delay in longArrayOf(2500, 5000)) {
            main.postDelayed({
                if (!credsRequested && lastPairStatus == -99 && gattClient != null) {
                    ...
                    gattClient?.writeCommand(
                        dev.pillar.osmodule.duml.OsmoCommands.setPairingPin(pairPin, identifier = pairIdentity())
                    )
                }
            }, delay)
        }
    ...
    override fun onNotification(sourceChar: java.util.UUID, raw: ByteArray, parsed: DjiMessage?) {
        ...
        if (parsed != null && parsed.flags == 0x40) {
            ...
            val respPayload = if (parsed.cmdSet == 0x00 && parsed.cmdId == 0x81)
                dev.pillar.osmodule.duml.OsmoCommands.APP_DEVICE_INFO else parsed.payload
        ...
        if (parsed != null && parsed.cmdSet == 0x51 && parsed.cmdId == 0x01 && bleDroneSerial == null) {
            dev.pillar.osmodule.drone.DroneSerial.inTunnelFrame(parsed.payload)?.let { (s, tag) ->
    ...
    companion object {
        ...
        private const val REMOTE_OWNER_HANDOFF_RETRIES = 20
        private const val REMOTE_OWNER_HANDOFF_RETRY_MS = 75L
        private const val GALLERY_CACHE_PREFS = "gallery_snapshot_cache"
