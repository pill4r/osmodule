 ...
 *    own; marked with [EXIF_THUMB][dev.pillar.osmodule.core.CameraFile.EXIF_THUMB] and read from a
...
class ImageLoader(private val http: HttpClient, private val log: (String) -> Unit) {
    ...
    fun load(thumbUrlPath: String, view: ImageView) {
        ...
        exec.submit {
            ...
            val exif = dev.pillar.osmodule.core.CameraFile.EXIF_THUMB
            var bytes = if (thumbUrlPath.startsWith(exif)) {
                http.getRange(thumbUrlPath.removePrefix(exif), 0,
                    (dev.pillar.osmodule.core.EmbeddedJpeg.HEAD_BYTES - 1).toLong())
                    ?.let { dev.pillar.osmodule.core.EmbeddedJpeg.fromHeader(it) }
            } else {
